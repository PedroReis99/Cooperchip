        ...
        private void TableHeader(TagHelperOutput output, PropertyInfo[] props)
        {
            ...
            foreach (var prop in props)
            {
                if (!prop.PropertyType.ToString().Contains("System.Collection"))
                {
                    ...
                    output.Content.AppendHtml(!name.Equals("Id") ? $"<th>{name}</th>" 
                        : $"<th>Ação</th>");
                }
            }
            output.Content.AppendHtml("</tr>");
            ...
            foreach (var item in Items)
            {
                ...
                foreach (var prop in props)
                {
                    ...
                    if (prop.Name.Equals("Id"))
                    {
                        ...
                        output.Content.AppendHtml($"<td><a href='/{model}/Details/{value}' " +
                            $"class='btn btn-default'>Detalhes</a>");
                        output.Content.AppendHtml($"&nbsp;<a href='/{model}/Edit/{value}' " +
                            $"class='btn btn-info'>Editar</a>");
                        output.Content.AppendHtml($"&nbsp;<a href='/{model}/Delete/{value}' " +
